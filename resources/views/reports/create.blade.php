...
@section('content')
    ...
    <form method="POST" action="{{ route('reports.store') }}" enctype="multipart/form-data" x-data="{
                              type: '{{ old('type', 'rc') }}',
                              rcDirections: {{ json_encode(old('rc_directions', [['arah_remote_control' => '', 'penyulang' => '']])) }},
                              rcCommissionings: {{ json_encode(old('rc_commissionings', [])) }},
                              pointsMap: @js($rcPoints->pluck('typeSignal.name', 'id')),
                              stdPoints: @js($rcPoints->pluck('id')),

                              init() {
                                  if (this.rcCommissionings.length === 0 && this.rcDirections.length > 0) {
                                      this.rcDirections.forEach((dir, i) => {
                                          this.syncPointsForDirection(dir.arah_remote_control, i);
                                      });
                                  }
                              },
                              ...
                              removeDirection(index) {
                                  ...
                                  this.rcCommissionings = this.rcCommissionings.filter(c => c._dir_id !== index);
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-xl p-8 shadow-sm">
                <h2 class="text-xl font-bold text-on-surface mb-6 font-display">Informasi Umum</h2>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Type Selection --}}
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-3">Tipe Laporan</label>
                        <div class="flex flex-wrap gap-4">
                            <label class="flex items-center gap-3 px-6 py-4 rounded-xl cursor-pointer transition-all border-2 border-transparent" :class="type === 'rc' ? 'bg-primary/5 border-primary/20 ring-4 ring-primary/5' : 'bg-surface-container-low hover:bg-surface-container'">
                                <input type="radio" name="type" value="rc" x-model="type" class="sr-only">
                                <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-colors" :class="type === 'rc' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-on-surface/5 text-on-surface/40'">
                                    <span class="material-symbols-outlined text-xl">settings_remote</span>
                                </div>
                                <div>
                                    <p class="text-sm font-bold transition-colors" :class="type === 'rc' ? 'text-primary' : 'text-on-surface/70'">Remote Control</p>
                                    <p class="text-[10px] opacity-50" :class="type === 'rc' ? 'text-primary' : 'text-on-surface'">Commissioning Report</p>
                                </div>
                            </label>

                            <label class="flex items-center gap-3 px-6 py-4 rounded-xl cursor-pointer transition-all border-2 border-transparent" :class="type === 'gfd' ? 'bg-secondary/5 border-secondary/20 ring-4 ring-secondary/5' : 'bg-surface-container-low hover:bg-surface-container'">
                                <input type="radio" name="type" value="gfd" x-model="type" class="sr-only">
                                <div class="w-10 h-10 rounded-lg flex items-center justify-center transition-colors" :class="type === 'gfd' ? 'bg-secondary text-white shadow-lg shadow-secondary/20' : 'bg-on-surface/5 text-on-surface/40'">
                                    <span class="material-symbols-outlined text-xl">electrical_services</span>
                                </div>
                                <div>
                                    <p class="text-sm font-bold transition-colors" :class="type === 'gfd' ? 'text-secondary' : 'text-on-surface/70'">Ground Fault Detector</p>
                                    <p class="text-[10px] opacity-50" :class="type === 'gfd' ? 'text-secondary' : 'text-on-surface'">Inspection Report</p>
                                </div>
                            </label>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', date('Y-m-d')) }}" required class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all">
                    ...
                    <div>
                        ...
                        <input type="text" name="gardu" value="{{ old('gardu') }}" required class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all" :placeholder="type === 'gfd' ? 'Nomor gardu...' : 'Nama gardu...'">
                    ...
                    <template x-if="type === 'gfd'">
                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
                            <div>
                                ...
                                <input type="text" name="arah_gardu" value="{{ old('arah_gardu') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-secondary/20 transition-all" >
                            ...
                            <div>
                                ...
                                <input type="text" name="task_type" value="{{ old('task_type') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-secondary/20 transition-all">
                            ...
                            <div>
                                ...
                                <input type="text" name="penyulang" value="{{ old('penyulang') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-secondary/20 transition-all">
                            ...
                            <div>
                                ...
                                <input type="text" name="gardu_induk" value="{{ old('gardu_induk') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-secondary/20 transition-all">
                            ...
                            <div>
                                ...
                                <input type="text" name="type_gardu" value="{{ old('type_gardu') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-secondary/20 transition-all">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" rows="2" required class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all resize-none" placeholder="Alamat lengkap lokasi..."></textarea>
                    ...
                    <div class="grid grid-cols-2 gap-4 md:col-span-2">
                        <div>
                            ...
                            <input type="text" name="latitude" value="{{ old('latitude') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="-6.200000">
                        ...
                        <div>
                            ...
                            <input type="text" name="longitude" value="{{ old('longitude') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="106.816666">
                    ...
                    <div class="md:col-span-2" x-show="type === 'rc'">
                        ...
                        <input type="text" name="notes" value="{{ old('notes') }}" class="w-full bg-surface-container-low rounded-md py-3.5 px-4 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all" placeholder="Catatan tambahan (opsional)...">
            ...
            <div class="bg-white rounded-xl p-8 shadow-sm">
                ...
                <div x-data="{ fileName: '' }">
                    <label class="relative group flex flex-col items-center justify-center w-full h-40 bg-surface-container-low rounded-2xl cursor-pointer hover:bg-primary/5 transition-all outline-none" style="border: 2px dashed rgba(124,58,237,0.15);">
                        <div class="absolute inset-0 bg-primary opacity-0 group-hover:opacity-[0.02] transition-opacity"></div>
                        <div class="text-center p-6">
                            <span class="material-symbols-outlined text-primary/40 text-4xl mb-3 group-hover:scale-110 transition-transform">cloud_upload</span>
                            <p class="text-xs text-on-surface/60 font-bold mb-1" x-text="fileName || 'Klik untuk pilih file'"></p>
                            <p class="text-[10px] text-on-surface/40 uppercase tracking-widest font-medium">XLSX, XLS (Max 5MB)</p>
                        </div>
            ...
            <div x-show="type === 'rc'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                ...
                <div class="bg-white rounded-xl p-8 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8 pb-8 border-b border-outline-variant/10">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-2">Arah</label>
                            <input type="text" name="arah" value="{{ old('arah') }}" class="w-full bg-surface-container-low rounded-md py-3 px-4 text-sm border-none" placeholder="Arah...">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-2">Lokasi</label>
                            <input type="text" name="lokasi" value="{{ old('lokasi') }}" class="w-full bg-surface-container-low rounded-md py-3 px-4 text-sm border-none" placeholder="Lokasi...">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-2">Tanggal Commissioning</label>
                            <input type="date" name="date_commissioning" value="{{ old('date_commissioning') }}" class="w-full bg-surface-container-low rounded-md py-3 px-4 text-sm border-none">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-2">RTU</label>
                            <input type="text" name="rtu" value="{{ old('rtu') }}" class="w-full bg-surface-container-low rounded-md py-3 px-4 text-sm border-none" placeholder="RTU...">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-on-surface/50 mb-2">Modem</label>
                            <input type="text" name="modem" value="{{ old('modem') }}" class="w-full bg-surface-container-low rounded-md py-3 px-4 text-sm border-none" placeholder="Modem...">
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left border-separate border-spacing-y-1" style="min-width: 1100px;">
                            <thead>
                                <tr class="text-[10px] font-bold text-on-surface/40 uppercase tracking-widest border-b border-outline-variant/10">
                                    <th class="pb-3 pr-2" rowspan="2">Database</th>
                                    <th class="pb-3 px-2" rowspan="2">Signal</th>
                                    <th class="pb-3 px-2" rowspan="2">Point Name</th>
                                    <th class="pb-3 px-2" rowspan="2">Terminal</th>
                                    <th class="pb-3 px-2 text-center" colspan="2">Signaling</th>
                                    <th class="pb-3 px-2 text-center" colspan="3">Control (DCC)</th>
                                    <th class="pb-3 px-2" rowspan="2">Arah</th>
                                    <th class="pb-3 px-2 text-center" rowspan="2">Status</th>
                                ...
                                <template x-for="(comm, ci) in rcCommissionings" :key="ci">
                                    <tr class="group hover:bg-surface-container-low shadow-sm transition-all rounded-lg overflow-hidden border border-outline-variant/10">
                                        ...
                                        <td class="py-3 px-1 text-center bg-surface-container-low/10 group-hover:bg-transparent border-l border-outline-variant/10">
                                            <input type="checkbox" x-model="comm.signaling_gh" :name="`rc_commissionings[${ci}][signaling_gh]`" class="rounded border-outline-variant text-primary focus:ring-primary/20 w-4 h-4">
                                        </td>
                                        <td class="py-3 px-1 text-center bg-surface-container-low/10 group-hover:bg-transparent">
                                            <input type="checkbox" x-model="comm.signaling_dcc" :name="`rc_commissionings[${ci}][signaling_dcc]`" class="rounded border-outline-variant text-primary focus:ring-primary/20 w-4 h-4">
                                        </td>
                                        {{-- Control --}}
                                        <td class="py-3 px-1 text-center bg-primary/5 group-hover:bg-transparent border-l border-outline-variant/10">
                                            <input type="checkbox" x-model="comm.control_dcc_rtu" :name="`rc_commissionings[${ci}][control_dcc_rtu]`" class="rounded border-outline-variant text-primary focus:ring-primary/20 w-4 h-4">
                                        </td>
                                        <td class="py-3 px-1 text-center bg-primary/5 group-hover:bg-transparent">
                                            <input type="checkbox" x-model="comm.control_dcc_rele_plat" :name="`rc_commissionings[${ci}][control_dcc_rele_plat]`" class="rounded border-outline-variant text-primary focus:ring-primary/20 w-4 h-4">
                                        </td>
                                        <td class="py-3 px-1 text-center bg-primary/5 group-hover:bg-transparent">
                                            <input type="checkbox" x-model="comm.control_dcc_lbs" :name="`rc_commissionings[${ci}][control_dcc_lbs]`" class="rounded border-outline-variant text-primary focus:ring-primary/20 w-4 h-4">
                                        </td>
                ...
                <div class="bg-white rounded-xl p-8 shadow-sm">
                    ...
                    <textarea name="commissioning_notes" rows="3" class="w-full bg-surface-container-low rounded-xl py-4 px-5 text-sm border-none focus:ring-2 focus:ring-primary/20 transition-all resize-none shadow-inner" placeholder="Penyimpangan atau catatan khusus selama proses commissioning..."></textarea>
                </div>
            </div>
                        ...
                        <table class="w-full text-left">
                            ...
                            <tbody class="divide-y divide-outline-variant/10">
                                @foreach($gfdItems as $index => $item)
                                    <tr class="hover:bg-surface-container-low/20 transition-colors" x-data="{ checked: '' }">
                ...
                <div x-data="{ 
                    ...
                    addSelection(event) {
                        ...
                        if (files.length > 0) {
                            ...
                            // Store the files and descriptions in Alpine
                            ...
                            // Move the current file input to a hidden container and replace it
                            const input = event.target;
                            const container = document.getElementById('hidden-inputs-container');
                            
                            // Give the input a unique ID so we can find it if needed
                            input.id = 'input-' + id;
                            ...
                            // Put the new input back in the label
                    ...
                    <div class="space-y-6">
                        ...
                        <label id="upload-label" class="relative group flex flex-col items-center justify-center w-full h-40 bg-surface-container-low rounded-2xl cursor-pointer hover:bg-primary/5 transition-all border-2 border-dashed border-outline-variant/30 overflow-hidden">
                            ...
                            <div class="text-center p-8">
                                ...
                                <p class="text-xs font-bold text-on-surface mb-1">Klik untuk Tambah Foto</p>
                                <p class="text-[10px] text-on-surface/40 uppercase tracking-widest font-medium">Bisa pilih multiple sekaligus</p>
                            </div>
                            <input type="file" name="images[]" multiple accept="image/*" class="hidden" @change="addSelection($event)">
                        </label>

                        {{-- Hidden container for the actual file inputs being submitted --}}
            ...
            <div class="flex items-center gap-4 pt-4">
                ...
                <a href="{{ route('reports.index') }}" class="h-14 px-8 text-on-surface-variant hover:text-on-surface font-bold text-sm transition-colors flex items-center gap-2">
